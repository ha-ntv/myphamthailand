...
$smarty->assign("loaikhachhang",$loaikhachhang);
switch($act){	
	case "dellist":
		case "dellist":
		if(!checkPermision($idpem,3)){
			page_permision();
			$page = "index.php?do=orders&s=".$_GET['s'];
			page_transfer2($page);
		}
		else{
			$id=$_POST["iddel"];
		...
		if(!checkPermision($idpem,2)){
			...
			for($i=0;$i<count($id);$i++){
				...
				*/
				...
				$sqlcrm = "SELECT * FROM $GLOBALS[db_crm].serial where idpr='".$rs_order['idpr']."' and code='".$rs_order['serial']."' ";
				$rscrm = $GLOBALS["crm"]->GetRow($sqlcrm);
				////Lưu history
				if(empty($rscrm['history']))
					$history = $rs_order['id'];
				else
					$history = $rs_order['id'].','.$rscrm['history'];
	...
	default:
		////////////Load thanh vien
		$sql = "select * from $GLOBALS[db_sp].member where type=1 order by name asc";
		$rs = $GLOBALS["sp"]->getAll($sql);
		...
		if($_GET['kh']==1){
			$wh .= " and loaikhachhang = 1 ";
		}
		if($_GET['kh']==2){
			$wh .= " and loaikhachhang = 2 ";
		}
		
		$template = "orders/list.tpl";
		$sql="select * from $GLOBALS[db_sp].orders where $wh $whCity order by id desc ";
