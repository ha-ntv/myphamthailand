...
<div class="WrapContent">
    <div class="Right">
        <div class="AllTitle">
            <div class="TitleLeft">
                ...
                <span class="subconten">		
					<a href="<!--{$path_url}-->/nha-cung-cap/<!--{$smarty.request.cat1}-->/0/">Nhà cung cấp</a> 
				</span>
                <span class="subconten"><img style="margin-top:9px" src="<!--{$path_url}-->/images/icon.gif"></span>
                <span class="subconten">		
					<!--{$smarty.request.cat3}-->
     ...
     <form name="allsubmit" id="frmEdit" action="<!--{$path_url}-->/nha-cung-cap/<!--{$smarty.request.cat1}-->/<!--{$smarty.request.cat2}-->/<!--{if $smarty.request.cat3 eq 'add'}-->addsm<!--{else}-->editsm<!--{/if}-->/" method="post" enctype="multipart/form-data">
		<div class="FormBox">
        	<div class="SubBox">
...
<script language="javascript">
	...
	function SubmitFrom(){
		...
		if(name.val().length == ''){
			...
			var id = $("#id").val();
			jQuery.post('<!--{$path_url}-->/ajax/Checkip.php',{name:name.val(),act:'checkPartner',id:id},function(data) {																				
