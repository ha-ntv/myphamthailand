...
switch($act){
	case "delete":
		if(!checkPermision($idpem,3)){
			page_permision();
			$page = $path_url."/khach-hang/".$_REQUEST['cat1']."/".$_REQUEST['cat2']."/";
			page_transfer2($page);
		}
		else{
			$id = trim($_REQUEST['cat4']);		
			$sql="delete from $GLOBALS[db_sp].khachhang where id=$id and idcity in (".$_SESSION['admin_showCity'].")";
			$GLOBALS["sp"]->execute($sql);
			
			$url = $path_url."/khach-hang/".$_REQUEST['cat1']."/".$_REQUEST['cat2']."/";
			page_transfer2($url);
		}
	break;
	...
	case "edit":
		if(!checkPermision($idpem,2)){
			page_permision();
			$page = $path_url."/khach-hang/".$_REQUEST['cat1']."/";
			page_transfer2($page);
		}
		else{
			$id  = $_GET["cat4"];
			$sql = "select * from $GLOBALS[db_sp].khachhang where id=$id and idcity in (".$_SESSION['admin_showCity'].")";
	...
	default:
		if(!checkPermision($idpem,5)){
			...
			$url = $path_url."/khach-hang/".$_REQUEST['cat1']."/0/0"; //set url for paginator
			$iSEGSIZE=10;
			$link_url = "";
...
$smarty->assign("checkKhachhang","class='active'");
...
function Editsm()
{
	...
	if ($act=="addsm")
	{
		vaInsert('khachhang',$arr); /// thêm dữ liệu đầu tiên
	}
	else
	{
		$id = trim($_POST['id']);
		//////////////////////////////////////////	
		vaUpdate('khachhang',$arr,' id='.$id);
	}
}
?>