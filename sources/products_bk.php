...
switch($act){
	...
	/*
		...
		*/	
		...
		";
		$rs = $GLOBALS["sp"]->getRow($sql);
		$checkPhuKien = checkPhuKien($rs['cid']);
		if($checkPhuKien > 1)
	...
	default:
		...
		if(empty($prid)){
			...
			if($strPrice=='new'){
				...
				$strPrice = 'moi';
				$smarty->assign("classnew",'class="active"');
			}
			else if($strPrice=='likenew'){
				$wh = " and pr.name_".$lang." LIKE '%hàng trưng bày%' ";
				$strPrice = 'cu';
				$smarty->assign("classold",'class="active"');
			}
			else if($strPrice=='world'){
				$wh = " and pr.name_".$lang." LIKE '%quốc tế%' ";
				$strPrice = 'quocte';
				$smarty->assign("classworld",'class="active"');
			}
			else if($strPrice=='lock'){
				$wh = " and pr.name_".$lang." LIKE '%lock%' ";
				$smarty->assign("classlock",'class="active"');
			}	
			$sql = "select * from $GLOBALS[db_sp].products pr, $GLOBALS[db_sp].colorsize cldd 
					...
					or ( pr.cid_sharepr like '%,".$cat['id'].",%' and pr.id=cldd.idpr and cldd.price_vn > 0 and cldd.idcity=$showCity )
					group by pr.id
