...
switch($act){
	...
	case "contact": 
		$arr = array();
		$arr['phone'] = $phone = striptags(isset($_POST['sender_telephone1'])?$_POST['sender_telephone1']:"");
		$idpr = trim(isset($_POST['idpr'])?$_POST['idpr']:"");
		$price = trim(isset($_POST['priceTotal'])?$_POST['priceTotal']:"");
		if(intval($idpr) > 0) $arr['idpr'] = $idpr;
		$arr['dated'] = time();
		$ContactID = vaInsert('contact',$arr);
		$sql = "select * from $GLOBALS[db_sp].infos where id=67 ";
		$rs = $GLOBALS["sp"]->getRow($sql);
		$smarty->assign("thanksyou",$rs);
		$template = "gio-hang/finish1.tpl";
			$BaseUrl = $path_url."/gio-hang/finish1/";
			echo"<script type=\"text/javascript\">
			        
			        localStorage.setItem('phone_price',".$price.");
			         localStorage.setItem('productID',".$idpr.");
	...
	case"thanh-toan":
		...
		$idcolorsize = striptags(isset($_POST['idcolorsize'])?$_POST['idcolorsize']:0);
		$idcolor = striptags(isset($_POST['idcolor'])?$_POST['idcolor']:0);
		...
		if($strPhone==0 && !empty($name) && is_numeric($phone) && $checkPhone>= 10 && $checkPhone<= 11 ){
			...
			$list .= '<td valign="top" bgcolor="#FFFFFF" align="left" >'.$row['name_'.$lang].$bonhoShow. ' ' . getNameAll($idcolor,'colorpr','name'). ' <br />'.$img.' <br /> '.$strgiahotro.' <br /> '.$vcareText.' </td>';
			...
			$arrOrder['idcolorsize'] = $idcolorsize;
			$arrOrder['idcolor'] = $idcolor;
		...
		*/
		...
		//	$BaseUrl = $path_url."/gio-hang/finish/".$OrderId."/";
			...
			echo"<script type=\"text/javascript\">
			        localStorage.setItem('id_tragop',".$order_id.");
