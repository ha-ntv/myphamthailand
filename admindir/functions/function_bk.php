...
function getNamePrSearch($type,$idpr){
	$sql = " select * from $GLOBALS[db_sp].products pr, $GLOBALS[db_sp].colorsize cldd 
					where pr.id= $idpr 
					and pr.id=cldd.idpr
					and cldd.idcity=1
...
function getIhubdanang($data)
{
    ...
    foreach ( $divs as $div )
    {
        if ( $div -> hasAttribute('class') && strpos( $div -> getAttribute('class'), 'price-gruop' ) !== false )
        {
				$price = $div -> nodeValue;
        }
    }
	$price = explode(':',strSpace($price));
...
function getClickbuy($data)
{
    ...
    foreach ( $divs as $div )
    {
        if ( $div -> hasAttribute('class') && strpos( $div -> getAttribute('class'), 'price_val' ) !== false )
        {
				$price = $div -> nodeValue;
        }
    }	
