<?php 
	...
	switch($type)
	{
		...
		case 'searchSubmenu':
			...
			else if($cid==35 || $cid==173){
				$sql = "select * from $GLOBALS[db_sp].products pr, $GLOBALS[db_sp].colorsize cldd 
						where pr.active=1 
						and pr.id=cldd.idpr
						and cldd.idbonho = 1
						and cldd.idcity=$showCity
						and  pr.cid in (select id from $GLOBALS[db_sp].categories where pid = ".$cid." and active=1) 
						$wh
						$whorder
				";
		...
		case 'priceBonho':
			...
			die(json_encode(array('priceBonho'=>$priceBonho,'priceAll'=>$priceAll,'nameBonho'=>$nameBonho)));
	 	break;
}
